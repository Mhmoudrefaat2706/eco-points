...
export class MaterialsDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    private materialsService: MaterialsService // Inject the service
  ) {}
  ...
  private loadMaterial(): void {
    // Get all materials from the service
    const allMaterials = this.materialsService.getAllMaterials();
    // Find the material with matching ID
    this.material = allMaterials.find(m => m.id === this.id);
  }
...
private getUserMaterials(): Material[] {
  ...
  try {
    return JSON.parse(storedMaterials).map((material: any) => ({
      ...
      priceUnit: material.priceUnit || 'piece'
    }));
  } catch (e) {
